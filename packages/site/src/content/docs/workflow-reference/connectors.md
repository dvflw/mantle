...
| `model` | string | Yes | Embedding model. OpenAI: `text-embedding-3-small`, `text-embedding-3-large`. Bedrock: `amazon.titan-embed-text-v2:0`, `amazon.titan-embed-text-v1`, `cohere.embed-english-v3`, `cohere.embed-multilingual-v3`. |
| `input` | string \| list | Yes | A single string or an array of strings to embed. |
| `dimensions` | integer | No | Requested embedding dimension, when the model supports it (OpenAI `text-embedding-3-*`, Bedrock `titan-embed-text-v2`). Cohere models have a fixed dimension (1024) and ignore this. |
| `input_type` | string | No | Cohere Bedrock models only: `search_document` (default), `search_query`, `classification`, or `clustering`. Embed documents at ingest with `search_document` and questions with `search_query`. Ignored by other providers/models. |
...
| `embedding_column` | string | No | Vector column name. Default `embedding`. |
| `filter` | object | No | Restrict the search to rows whose metadata contains these key/values (JSONB `@>` containment). E.g. `{ source: confluence, team: a }`. An empty object is ignored. |
| `metadata_column` | string | No | JSONB column the `filter` matches against. Default `metadata`. |
...
- name: search
  ...
  params:
    ...
    top_k: 5
    # Optional: only search rows whose metadata contains these fields.
    filter:
      source: confluence
```

The `filter` is applied with the JSONB containment operator (`@>`), so it matches rows whose metadata is a superset of the object you pass — exact key/value matches, including nested objects and array membership. The whole filter is bound as a single parameter (no SQL injection surface).
...
| `unit` | string | No | `chars` (default, Unicode-aware), `words` (whitespace-separated), or `recursive` (see below). |
