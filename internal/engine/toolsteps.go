...
func (ts *ToolSteps) CacheLLMResponse(ctx context.Context, stepID string, response map[string]any) error {
	...
	result, err := ts.DB.ExecContext(ctx, `
		...
		SET cached_llm_responses = COALESCE(cached_llm_responses, '[]'::jsonb) || $2::jsonb
		WHERE id = $1
	`, stepID, data)
	if err != nil {
...
func (ts *ToolSteps) LoadCachedLLMResponses(ctx context.Context, stepID string) ([]map[string]any, error) {
	var raw []byte
	err := ts.DB.QueryRowContext(ctx, `
		SELECT COALESCE(cached_llm_responses, '[]'::jsonb) FROM step_executions WHERE id = $1
	`, stepID).Scan(&raw)
	if err != nil {
...
func (ts *ToolSteps) LoadSubStepStatuses(ctx context.Context, parentStepID string) (map[string]*SubStepStatus, error) {
	...
	statuses := make(map[string]*SubStepStatus)
	for rows.Next() {
		var (
			stepName  string
			status    string
...
// SubStepStatus holds the status of a tool sub-step execution.
